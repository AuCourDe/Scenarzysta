...
class DocumentProcessor:
    ...
    def stage1_generate_test_paths(self, extracted_data: Dict, processing_dir: Path) -> List[Dict]:
        ...
        full_prompt = f"{prompt_template}\n\nDOKUMENTACJA:\n{doc_text}\n\nPAMIĘTAJ: Zwracasz TYLKO JSON (bez żadnego tekstu przed lub po). Musisz wygenerować 30-50 ścieżek testowych."
        
        # Wywołaj Ollama
        print("ETAP 1: Generowanie ścieżek testowych...")
        response = self._call_ollama(full_prompt)
        
        if not response or len(response.strip()) < 10:
            raise Exception("Ollama zwróciła pustą odpowiedź w etapie 1")
            ...
            for i, path in enumerate(paths):
                if isinstance(path, dict):
                    ...
                    if 'id' in path and 'title' in path:
                        # Upewnij się, że source_sections istnieje (konwersja z source_pages jeśli potrzeba)
                        if 'source_sections' not in path:
                            if 'source_pages' in path:
                                # Konwersja starych danych: numery stron -> nazwy sekcji
                                path['source_sections'] = []
                            else:
                                path['source_sections'] = []
                        valid_paths.append(path)
                    else:
                        print(f"  Ostrzeżenie: Ścieżka {i} nie ma wymaganych pól (id, title), pomijam")
                else:
                    print(f"  Ostrzeżenie: Ścieżka {i} nie jest słownikiem, pomijam")
            
            if len(valid_paths) == 0:
                raise Exception(f"Nie wygenerowano żadnych ścieżek testowych. Odpowiedź Ollama: {response[:500]}")
            
            if len(valid_paths) < 10:
                print(f"  UWAGA: Wygenerowano tylko {len(valid_paths)} ścieżek, oczekiwano 30-50")
            
            # Zapisz ścieżki do pliku
            paths_file = processing_dir / "sciezki_testowe.txt"
            with open(paths_file, 'w', encoding='utf-8') as f:
                json.dump(valid_paths, f, ensure_ascii=False, indent=2)
            
            print(f"ETAP 1: Wygenerowano {len(valid_paths)} ścieżek testowych")
            return valid_paths
            
        except json.JSONDecodeError as e:
            print(f"Błąd parsowania JSON z etapu 1: {e}")
            print(f"Odpowiedź Ollama: {response[:500]}")
            raise Exception(f"Nie udało się sparsować odpowiedzi z etapu 1: {e}")
    ...
    def stage2_generate_scenarios(self, extracted_data: Dict, test_paths: List[Dict], processing_dir: Path) -> List[Dict]:
        ...
        full_prompt = f"{prompt_template}\n\nDOKUMENTACJA:\n{doc_text}\n\nŚCIEŻKI TESTOWE:\n{paths_json}\n\nPAMIĘTAJ: Zwracasz TYLKO JSON (bez żadnego tekstu przed lub po). Musisz wygenerować 50-70 scenariuszy testowych. Dla każdej ścieżki utwórz scenariusze pozytywne i negatywne."
        
        # Wywołaj Ollama
        print("ETAP 2: Generowanie scenariuszy testowych z walidacjami...")
        response = self._call_ollama(full_prompt)
        
        if not response or len(response.strip()) < 10:
            raise Exception("Ollama zwróciła pustą odpowiedź w etapie 2")
        ...
        try:
            ...
            for i, scenario in enumerate(scenarios):
                if isinstance(scenario, dict):
                    ...
                    if 'scenario_id' in scenario and 'title' in scenario:
                        ...
                        print(f"  Ostrzeżenie: Scenariusz {i} nie ma wymaganych pól (scenario_id, title), pomijam")
                else:
                    print(f"  Ostrzeżenie: Scenariusz {i} nie jest słownikiem, pomijam")
            
            if len(valid_scenarios) == 0:
                raise Exception(f"Nie wygenerowano żadnych scenariuszy testowych. Odpowiedź Ollama: {response[:500]}")
            
            if len(valid_scenarios) < 30:
                print(f"  UWAGA: Wygenerowano tylko {len(valid_scenarios)} scenariuszy, oczekiwano 50-70")
            ...
            print(f"ETAP 2: Wygenerowano {len(valid_scenarios)} scenariuszy testowych")
            return valid_scenarios
            
        except json.JSONDecodeError as e:
            print(f"Błąd parsowania JSON z etapu 2: {e}")
            print(f"Odpowiedź Ollama: {response[:500]}")
            raise Exception(f"Nie udało się sparsować odpowiedzi z etapu 2: {e}")
