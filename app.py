...
def process_tasks_worker():
    ...
    while not stop_processing:
        ...
        try:
            ...
            # Analiza multimodalna (symulacja - w rzeczywistości użyj modelu wizyjnego)
            task_queue.update_progress(task.task_id, 50.0)
            ...
            task_queue.update_progress(task.task_id, 80.0)
            test_scenarios = document_processor.generate_test_scenarios(
                analyzed_data
            )
            
            # Zapis wyników
            task_queue.update_progress(task.task_id, 90.0)
            result_file = document_processor.save_results(
                test_scenarios,
                results_dir,
                task.task_id
            )
