...
class TaskQueue:
    ...
    def get_queue_status(self, user_id: Optional[str] = None) -> Dict:
        ...
        Args:
            user_id: Jeśli podany, zwraca tylko zadania tego użytkownika
        ...
        with self._lock:
            if user_id:
                user_tasks = [task for task in self._tasks.values() 
                             if task.user_id == user_id]
                pending_tasks = [t for t in user_tasks if t.status == TaskStatus.PENDING]
                processing_tasks = [t for t in user_tasks if t.status == TaskStatus.PROCESSING]
            else:
                pending_tasks = [t for t in self._tasks.values() 
                               if t.status == TaskStatus.PENDING]
                processing_tasks = [t for t in self._tasks.values() 
                                  if t.status == TaskStatus.PROCESSING]
            
            # Oblicz pozycje w kolejce
            queue_positions = {}
            ...
            # Oblicz szacowany czas dla zadań użytkownika przed jego zadaniem
            user_wait_time = 0.0
            if user_id:
                for tid in self._queue:
                    ...
                    if task.task_id == self._current_task_id:
                        # To aktualnie przetwarzane zadanie
                        ...
                        # To zadanie użytkownika - dodaj jego czas
                        user_wait_time += task.estimated_duration or 0
                    elif task.status == TaskStatus.PENDING:
                        # To zadanie innego użytkownika przed zadaniem użytkownika
                        user_wait_time += task.estimated_duration or 0
            ...
            result = {
                ...
                "user_wait_time": user_wait_time if user_id else None,
                "tasks": []
            }
            
            # Dodaj zadania do wyniku
            all_tasks = user_tasks if user_id else list(self._tasks.values())
    ...
    def _estimate_duration(self, file_size: int) -> float:
        ...
        """
        # Bazowa estymacja: 1 MB = ~30 sekund
        base_estimate = (file_size / (1024 * 1024)) * 30
        
        # Jeśli mamy historię, użyj średniej z podobnych plików
        if self._history:
            similar_tasks = [
                h for h in self._history
                if abs(h['file_size'] - file_size) / max(file_size, 1) < 0.5
            ]
    ...
    def _add_to_history(self, task: Task):
        ...
        if task.actual_duration and task.estimated_duration:
            # Próbuj oszacować rozmiar pliku na podstawie czasu
            # (w rzeczywistości powinniśmy to przechowywać w Task)
            estimated_size = (task.estimated_duration / 30) * (1024 * 1024)
